...
self.addEventListener("install", e => {
  ...
  e.waitUntil(caches.keys()
    .then(k => Promise.all(k.map(c => {
      if (c !== CACHE_NAME) return caches.delete(c)
    })))
    .then(() => self.clients.claim()))
}), self.addEventListener("fetch", e => {
  if ("GET" === e.request.method && e.request.url.startsWith(self.location.origin)) {
    e.respondWith(caches.match(e.request, {
        ignoreSearch: !0
      })
      .then(r => {
        const f = fetch(e.request)
          .then(n => {
            if (n && 200 === n.status && "basic" === n.type) {
              const l = n.clone();
              caches.open(CACHE_NAME)
                .then(c => c.put(e.request, l))
            }
            return n
          })
          .catch(() => {
